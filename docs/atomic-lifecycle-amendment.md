...
| A token, router, adapter, guard, or AMM dependency is an EOA or other code-less address | Manager or factory construction reverts before an unusable direct deployment or bundle can persist; the v4 factory separately requires PoolManager code even when the supplied expected hash matches the empty account's code hash, and the deployment script rejects configured code-less dependencies before broadcast. |
...
| A single-leg v4 donation accrues before an unresolved partial redemption | The redeemer receives only its floor-rounded share as an unmatched outcome token in kind; its base balances do not change unless it later redeems a winning token, and survivors retain the remainder. |
...
| Outcome wrappers arrive after the final share burn | With zero shares outstanding, the owner may sweep only the captured wrapper balances to the bootstrap recipient. Every transfer requires the exact recipient delta; failure on any wrapper restores all earlier wrapper payments before an identical retry. |
...
| Owner becomes unavailable after arming emergency exit | After the fixed delay, any account can unwind positions into the manager; settlement of the captured CTF condition and share redemption remain permissionless, and no caller receives shareholder assets. Unit tests and the randomized handler cover settlement after permissionless execution; the pinned-mainnet fixture additionally faults the second removal after the first official-v4 unwind, proves complete rollback, then proves zero outsider gain, proportional shareholder redemption while canonical CTF is unresolved, source-independent survivor settlement, and aggregate final conservation. |
| Emergency is armed before any proposal activates | After the delay, any account can remove the live spot position into the manager without receiving shareholder assets. Pinned-mainnet runs separately fault official-v3 principal removal after fee collection and NFT burn after fee collection, full principal removal, and principal collection. Both prove the NFT/liquidity, custody, shares, and emergency flag restore, then complete the identical outsider retry with zero gain and recover both bootstrap assets within two wei. |
...
- fault each in-kind wrapper transfer after merge fallback and prove preceding wrapper payments plus
