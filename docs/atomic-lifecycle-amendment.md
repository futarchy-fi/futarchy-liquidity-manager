...
| Emergency is armed before any proposal activates | After the delay, any account can remove the live spot position into the manager without receiving shareholder assets. The pinned-mainnet fixture proves the outsider removes the official-v3 NFT with zero gain and no share mutation, after which final shareholder redemption recovers both bootstrap assets within two wei. |
