...
| Emergency is armed before any proposal activates | After the delay, any account can remove the live spot position into the manager without receiving shareholder assets. The pinned-mainnet fixture faults official-v3 principal removal after fee collection, proves the NFT/liquidity, custody, shares, and emergency flag restore, then completes the identical outsider retry with zero gain and recovers both bootstrap assets within two wei. |
