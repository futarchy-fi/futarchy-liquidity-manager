...
- Partial redemption removes proportional spot, YES, and NO liquidity; accounts for principal,
  ...
  position liquidity.
- Independent company-token and wrapped-collateral payout faults after all three proportional
  removals and both complete-set merges roll back the share burn, manager and adapter liquidity,
  removal counters, and six-token manager/adapter/router/recipient custody. The second fault also
  rolls back the recipient's preceding company-token transfer. Clearing the faults lets the
  identical redemption succeed.
...
- The current 255-test instrumented suite also passes Foundry's coverage profile with `--ir-minimum`
...
- The current 256-test normal suite includes direct emergency-handler reachability, ten
...
| Either final recipient payout during proportional redemption | `test_each_final_payout_failure_rolls_back_complete_conditional_redemption` independently faults the exact company-token and wrapped-collateral transfers after spot/YES/NO removal, share burn, and both complete-set merges; the latter occurs after company payment, and each restores shares, liquidity, removal counters, and six-token custody before the identical retry succeeds. |
