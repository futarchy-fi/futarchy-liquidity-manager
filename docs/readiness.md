...
- The exact CI coverage command passes 264 tests: every normal-profile test except the
  ...
  instrumentation exceeds Solidity's stack limit. The manager reports 92.48% line, 90.82%
  statement, 68.57% branch, and 98.65% function coverage; IR-minimum source mappings are informative
  rather than a production-bytecode identity check. Production compilation independently confirms
  22,091-byte creation code and a 19,821-byte manager runtime, leaving 4,755 bytes below EIP-170 and
  3,731 bytes below the enforced 23,552-byte project ceiling.
- The current 265-test normal suite includes direct emergency-handler reachability, ten
  ...
  282,624 calls with zero reverts. The malformed ERC20 balance-return rollback, executable
  deployment-reserve gate, artifact drift, permissionless bundle interleaving, code-less
  PoolManager, and immutable spot-tick-policy checks remain green.
- The remaining CI matrix also passes: production build and format, shell syntax, scope and API
  freeze, example deployment/v4-factory/batch schemas, every batch template, readiness evidence,
  and the limited-deployment preflight. Network publication and final-config fork repetition remain
